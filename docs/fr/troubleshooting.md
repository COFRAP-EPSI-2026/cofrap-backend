...
Les images `ghcr.io/cofrap-epsi-2026/<function>:<version>` ne sont publiées que sur **tag git `v*.*.*`** (via le workflow [`release.yml`](../../.github/workflows/release.yml)). Sur un fork ou avant de pousser un tag, ces images n'existent pas → ImagePullBackOff au démarrage des pods.
