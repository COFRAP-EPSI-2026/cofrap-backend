...
The `ghcr.io/cofrap-epsi-2026/<function>:2026.1.1` images are only published on a **git tag `v*.*.*`** (via the [`release.yml`](../../.github/workflows/release.yml) workflow). On a fork, or before pushing a tag, those images don't exist → ImagePullBackOff when the pods start.
